...
- **Scope filtering**: `impeccable` was project-scoped to AutomaticTrading (root cause: the agents-store copy was an incomplete stub; only the AutomaticTrading project had the complete v3.5.0 skill). **Resolved 2026-07-29**: the complete v3.9.1 skill now lives in `~/.agents/skills/impeccable` and is symlinked into `~/.claude/skills/impeccable` + `~/.codex/skills/impeccable`, so the manifest reports `scope: "global"` and the router recommends it globally. No `scope != "project"` filter needed for `impeccable` anymore. (General rule still holds: skills that are genuinely `scope: "project"` must be filtered out of global recommendations.)
...
| **Globally recommending `impeccable`** | Resolved 2026-07-29: complete v3.9.1 now in `~/.agents/skills/impeccable`, symlinked into claude + codex, manifest `scope: "global"`. | No longer filtered — recommended globally; mapped in `mode-map.json`. (Generic rule still applies to genuinely `scope: "project"` skills.) |
